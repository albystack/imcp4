...
class Trader:
    ...
    def run(self, state):
        ...
        self._update_counterparty_signals(memory, state)
        ...
        for product in self.UNDERLYINGS:
            if product == "VELVETFRUIT_EXTRACT" and state.timestamp < self.VE_WARMUP_TIMESTAMP:
                continue
            if product in state.order_depths:
                result[product].extend(self._trade_underlying(product, state, memory))

        option_fairs, option_deltas, fit_sigma = self._option_fair_values(state, memory)
        if option_fairs:
            for product in self.TRADE_VOUCHERS:
                if product in state.order_depths and product in option_fairs:
                    result[product].extend(self._trade_voucher(product, state, option_fairs[product], option_deltas[product], memory))

        if fit_sigma is not None:
            memory["sig"] = round(fit_sigma, 4)
        trader_data = self._encode_memory(memory)
        logger.print("ts", state.timestamp, "sig", memory.get("sig", ""), "hp", round(memory["sigv"].get("HYDROGEL_PACK", 0.0), 2), "ve", round(memory["sigv"].get("VELVETFRUIT_EXTRACT", 0.0), 2))
    ...
    def _decode_memory(self, trader_data):
        ...
        for part in trader_data.split(";"):
            ...
            if key == "HP":
                ...
                memory["sigv"]["HYDROGEL_PACK"] = number
            elif key == "SVE":
                memory["sigv"]["VELVETFRUIT_EXTRACT"] = number
            elif key == "SV4":
                memory["sigv"]["VEV_4000"] = number
        return memory

    def _encode_memory(self, memory):
        ema = memory.get("ema", {})
        sigv = memory.get("sigv", {})
        parts = []
        ...
        if abs(sigv.get("HYDROGEL_PACK", 0.0)) > 0.001:
            parts.append("SHP=" + str(round(sigv.get("HYDROGEL_PACK", 0.0), 4)))
        if abs(sigv.get("VELVETFRUIT_EXTRACT", 0.0)) > 0.001:
            parts.append("SVE=" + str(round(sigv.get("VELVETFRUIT_EXTRACT", 0.0), 4)))
        if abs(sigv.get("VEV_4000", 0.0)) > 0.001:
            parts.append("SV4=" + str(round(sigv.get("VEV_4000", 0.0), 4)))
        return ";".join(parts)
    ...
    def _update_counterparty_signals(self, memory, state):
        sigv = memory["sigv"]
        for product in ("HYDROGEL_PACK", "VELVETFRUIT_EXTRACT", "VEV_4000"):
            sigv[product] = 0.0

    def _mark_direction(self, product, mark, side):
        if product == "HYDROGEL_PACK":
            ...
            if mark == "Mark 55":
                return -side
            if mark == "Mark 67" and side > 0:
                return 0.5
            if mark == "Mark 49" and side < 0:
                return 0.5
    ...
    def _trade_underlying(self, product, state, memory):
        ...
        signal = memory.get("sigv", {}).get(product, 0.0)
        if product == "HYDROGEL_PACK":
            fair = ema + 2.0 * imbalance + signal
            take_edge = 8.0
            make_edge = 6.0
            base_size = 14
        else:
            fair = ema + 0.9 * imbalance + signal
            take_edge = 3.0
            make_edge = 2.0
            base_size = 20
    ...
    def _option_fair_values(self, state, memory):
        ...
        for product in market:
            ...
            fair = self._black_scholes_call(spot, strike, tte, fair_iv)
            if product == "VEV_4000":
                fair += memory.get("sigv", {}).get("VEV_4000", 0.0)
            fairs[product] = fair
    ...
    def _trade_voucher(self, product, state, fair, delta, memory):
        depth = state.order_depths[product]
        best_bid, best_ask = self._best_bid_ask(depth)
        if best_bid is None or best_ask is None:
            return []
        ...
        max_take = 18 if product in ("VEV_4000", "VEV_5300", "VEV_5400", "VEV_5500") else 12
        ...
        if buy_quote < sell_quote:
            base_size = 12 if product in ("VEV_4000", "VEV_5300", "VEV_5400", "VEV_5500") else 6
            skew = projected_position / self.ACTIVE_LIMITS[product]
